...
- Stage 4 release engineering: extended `.goreleaser.yaml` with a second `lazytg-sqlcipher` build entry (CGo, `darwin/{amd64,arm64}` + `linux/amd64`, opt-in via removing `SKIP_SQLCIPHER`), `nfpms` section producing `.deb`/`.rpm` packages from the pure-Go matrix, `brews` section auto-publishing the formula to `pgmac/homebrew-lazytg` on stable tags, per-archive cosign sigstore-bundle signing alongside the existing `checksums.txt` signature.
- Stage 4 pre-release pipeline: tag-driven gating in `.github/workflows/release.yml` detects `v*-{alpha,beta,rc}.*` suffixes and propagates `prerelease=true`/`skip_brew=true` to goreleaser via env; `release.prerelease: auto` plus `brews[].skip_upload: '{{ if .Prerelease }}true{{ end }}'` keep the Homebrew tap pinned to stable releases. New `.github/workflows/prerelease.yml` adds a manual `workflow_dispatch` entry point that auto-increments and pushes the next `alpha`/`beta`/`rc` tag.
