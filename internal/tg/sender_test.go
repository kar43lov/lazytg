...
func TestConvertMessage_PrivateChatKeepsItsSender(t *testing.T) {
	...
	in := convertMessage(privateMsg(28, peer, "йцу", false), peer, nil, nil)
	if in.FromID != peer {
	...
	out := convertMessage(privateMsg(29, peer, "asd", true), peer, nil, nil)
	if !out.Outgoing {
...
func TestConvertMessage_GroupSenderIsUntouched(t *testing.T) {
	...
	got := convertMessage(m, chatID, nil, nil)
