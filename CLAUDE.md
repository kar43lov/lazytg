...
- **Snapshot собирает с `--parallelism 2`, а не по числу ядер.** На четырёх параллельных сборках джоба дважды подряд умирала молча — ни ошибки goreleaser, ни строки в логе, только оборванный шаг; на двух проходит за ~3m50s. Причина не доказана (пик памяти не снят), поэтому в джобе стоит диагностика `nproc`/`free`/`df` до и после: следующее падение такого вида назовёт себя само.
...
**Состояние (19.08.2026):** два живых прогона на тестовом аккаунте позади — базовый путь и разрыв связи, — и каждый вскрыл дефекты, невидимые тестам. Разбор в auto-memory `lazytg_release_status`. В `main`: мышь, выделение текста, перетаскиваемый разделитель, циклический переход по чатам. Открыт PR #24 (отметки о прочтении, удаления с других устройств, имя чата, созданного live-путём) — CI зелёный, но **независимого ревью на нём не было**: 19.08 Codex и pi выбили лимиты одновременно.

**Первый релиз выпущен 18.08.2026: `v0.1.0-alpha.1`** (prerelease). Работают архивы, deb/rpm и `go install`; **`brew` НЕ работает** — формула публикуется только со stable-тега (`skip_upload` при `.Prerelease`), tap пуст. Детали пайплайна и проверки артефакта — в auto-memory `lazytg_release_status`. Правило для доков: **сверять обещания с реальным состоянием** — они дважды разошлись с ним за одну сессию, сначала обещая несуществующие релизные пути, потом утверждая «релиза нет» уже после тега.
