...
**Состояние (04.09.2026):** четыре живых прогона на тестовом аккаунте позади — базовый путь, разрыв связи и два обычных сеанса, — и каждый вскрыл дефекты, невидимые тестам. Разбор в auto-memory `lazytg_release_status`. Всё влито в `main`: мышь, выделение текста, перетаскиваемый разделитель, циклический переход по чатам, обе партии PR #24 (отметки о прочтении, удаления с других устройств, имя live-чата; подписи отправителей, счётчик непрочитанных, курсор списка — миграция `0010`). 🔴 **Независимого ревью не было ни на одной из партий PR #24** — 19.08 Codex и pi выбили лимиты одновременно, и код ушёл в `main` за одной подписью Claude. Лимиты давно сняты, долг открыт.
