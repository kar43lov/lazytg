...
**Первый релиз выпущен 18.08.2026: `v0.1.0-alpha.1`** (prerelease). Пайплайн отработал полностью — 4 архива, deb/rpm, checksums, cosign keyless; скачанный артефакт проверен: сумма сходится, `api: none`. Работают архивы, deb/rpm и `go install` (в том числе `@latest` — стабильных версий нет, proxy отдаёт alpha; версия при этом печатается как `dev`, ldflags ставит только goreleaser). **`brew` НЕ работает**: формула публикуется только со stable-тега (`skip_upload` при `.Prerelease`), tap `kar43lov/homebrew-lazytg` пуст. Secret `HOMEBREW_TAP_GITHUB_TOKEN` записан и проверен (API отдаёт `push: true`). Правило для доков остаётся прежним, но с обратным знаком: **сверять обещания с реальным состоянием** — в этой сессии доки дважды разошлись с ним, сначала обещая несуществующие релизные пути, потом продолжая утверждать «релиза нет» после тега.
