...
- 🔴 **Поле теряется между проводом и панелью, и фикстура это прячет — дважды за проект.** Сначала `makeMsg` в `internal/tg/history_test.go` звал `SetFromID` и для приватного чата, где Telegram `from_id` не присылает вовсе: оба конвертера читали пустое поле, личные сообщения уходили в зеркало с отправителем 0 и рисовались как `system`. Потом `events.MessageReceived` не нёс `ReplyTo` — конвертер его читал, а событие выбрасывало, и живой ответ рисовался без цитаты до переоткрытия чата. Класс один: **`domain.Message` и событие шины — два разных набора полей**, а каждая фикстура собирает сообщение структурой напрямую и мимо события. Добавляешь поле в `domain.Message` — проверь `events.MessageReceived`, `LiveService.persist` и `thread.applyIncoming`; фикстуру сверяй с тем, что реально приходит по проводу.
...
**Состояние (05.09.2026):** пять живых прогонов на тестовом аккаунте позади, и каждый вскрыл дефекты, невидимые тестам. Разбор в auto-memory `lazytg_release_status`. В `main` влито всё августовское и PR #24/#33/#34. 🔴 **Не влито: ветка `feat/message-actions`, 14 коммитов, ~11.8k строк, локально и без пуша** — действия над сообщениями (выделение/копирование/правка/удаление/пересылка), реакции (миграция `0012`), папки, inline-фото по Kitty-протоколу, эмодзи (панель + `:shortcode` на Tab), typing-индикатор, переход по ответам, линия непрочитанного, волна голосового (миграция `0013`), `Alt+1..9`. Живьём не проверялась ни одна из них. 🔴 **Независимого ревью не было ни на PR #24/#33, ни на этой ветке** — Codex в `usage limit` до 07.09 07:00, pi — до ≈06.09; всё за одной подписью Claude. Долг открыт и растёт.
