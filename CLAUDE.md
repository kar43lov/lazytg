...
- 🔴 **Фикстура, задающая поле, которого сервер не присылает, прячет целый класс дефектов.** `makeMsg` в `internal/tg/history_test.go` звал `SetFromID` и для приватного чата — а в 1:1 Telegram `from_id` опускает вовсе (отправитель выводится из `out` и peer). Поэтому оба конвертера читали пустое поле, все личные сообщения уходили в зеркало с отправителем 0 и рисовались как `system`, и ни один тест этого не видел при покрытии core в 81%. Правило: фикстуру для tg-конвертера сверять с тем, что реально приходит по проводу, а не с тем, что удобно собрать структурой.
...
- **Inline media:** рисования картинки в терминале нет и в v0.1 не будет (→ v0.3). Вложение — бейдж вида `[⏺ video note, 0:07, 1.2 MiB] o to open, ctrl+d to save`; смотрят его системным просмотрщиком по `o` или клику по бейджу. Для видео и кружочков это не заглушка, а единственный вариант: терминал видео не играет.
...
**Состояние (04.09.2026):** пять живых прогонов на тестовом аккаунте позади — базовый путь, разрыв связи, два обычных сеанса и прогон по медиа, — и каждый вскрыл дефекты, невидимые тестам. Разбор в auto-memory `lazytg_release_status`. Всё влито в `main`: мышь, выделение текста, перетаскиваемый разделитель, циклический переход по чатам, обе партии PR #24 (отметки о прочтении, удаления с других устройств, имя live-чата; подписи отправителей, счётчик непрочитанных, курсор списка — миграция `0010`) и PR #33 (курсор по сообщениям, `o`/клик открывают вложение системным просмотрщиком, виды вложений с длительностью — миграция `0011`, разделители дней, композер до четырёх строк). 🔴 **Независимого ревью не было ни на PR #24, ни на PR #33** — 19.08 Codex и pi выбили лимиты одновременно, 04.09 Codex снова в `usage limit` до 07.09 07:00, pi — до ≈06.09. Код ушёл в `main` за одной подписью Claude; долг открыт.
