...
| `internal/core/domain`           | 1     | Plain Go types: `Account`, `Chat`, `Message`, `MediaInfo`, `Reaction`, `Entity`, plus the stored codecs for the reaction and entity columns. | stdlib only                             | —                                        |
| `internal/core/markdown`         | 1     | Telegram Desktop's markup ⇄ rune-indexed entities (`Parse`, `Render`). Used by the send edge and the edit service; the pane uses it to preview an in-flight row. | `internal/core/domain`                  | everything else                          |
...
| `internal/core/notify`           | 1     | Desktop notifications behind `LAZYTG_NOTIFY=desktop`: `terminal-notifier` / `osascript` / `notify-send` or an override, text as arguments only.  | `log/slog`                              | everything else                          |
| `internal/storage/sqlite`        | 1–3   | Repository implementation: migrations 0001–0019, FTS5 trigram index, frecency, dedup tables, outgoing/peers/state repos.            | `internal/core/domain`                  | `internal/ui`, `internal/tg`             |
...
| `internal/ui/emoji`              | 4     | The emoji table (generated from Unicode by `scripts/gen_emoji.py`) plus ranked search, behind both the composer's `:shortcode` completion and the picker overlay. | stdlib only                             | everything else                          |
...
- The degradation probe (`Repo.ProbeWrite`) treats `SQLITE_BUSY` as success. It asks "does storage accept writes", and another connection holding the write lock answers yes. Treating contention as failure let `DegradationDetector` flip the repo into soft read-only mode — every write returning `ErrReadOnly` — whenever a probe landed inside a write burst. Nothing previously detected is lost: no genuine failure code masks to BUSY (5) or LOCKED (6), so any error that does reach the probe is still reported. Which failures reach it is a narrower set than the doc comment used to imply.
- The probe runs `DELETE FROM chats WHERE 0` inside its transaction, and that statement is the whole point of it. `BEGIN IMMEDIATE` alone takes the write lock without dirtying a page, so SQLite had no reason to raise `SQLITE_READONLY` and the probe called a read-only file healthy while every ordinary write failed with "attempt to write a readonly database" — `DegradationDetector` missed the one condition it was written for. `WHERE 0` can never match a row, but the statement still opens a write cursor, which is what raises code 8. Pinned by `TestProbeWrite_DetectsAReadOnlyDatabase` and `TestProbeWrite_LeavesNoRowsBehind`; the latter exists because a probe that deletes is a probe that must be proven not to delete anything.
...
- gotd's `updates.Manager` is the client's update handler, backed by `sqlite.StateRepo` for pts/qts/seq and by the peer table for channel access hashes (the manager loads a channel's stored pts only when it can also find its access hash, so an in-memory hasher would silently reduce recovery to private chats). `MaxChannelDifferenceConcurrency` is capped at 4: gap recovery costs one `updates.getChannelDifference` per channel, and firing all of them the instant a connection returns is both rate-limit bait and a traffic shape no human client produces.
- The manager degrades rather than fails. Its `Handle` forwards updates as-is when `Run` has not succeeded, so a state-storage or `getState` failure costs ordering and gap recovery, not live updates — `cmd/lazytg/cmd/session.go` logs it and holds the session open.
- 🔴 That degradation hid a total failure for as long as the manager existed. `state`/`channel_state` keyed on `account_id` with a foreign key to `accounts(id)`, while gotd keys its state storage by the **Telegram user id** — so every write failed the constraint, `Run` never succeeded, and every launch ran without gap recovery behind a single warning line. Migration `0009` keys both tables by the Telegram user id and drops the foreign key; accounts are separated by data directory, so no scoping column is needed. The lesson is about the degradation, not the schema: a fallback this quiet needs its failure counted somewhere a person looks.
- `--polling` engages a periodic pull of the three most recently active chats (3 s cadence, `internal/tg/polling.go`) *alongside* the push path, as a net for connections that drop updates silently. It is opt-in because it is steady background traffic on an account that is already watched more closely for running an unofficial client. Both paths publish through one duplicate filter (`UpdatesDispatcher.SeenMessage`): watermarks alone cannot close the overlap, since the polling source reads the store's newest id and only then makes its network call.
