...
| The account                 | `messages.searchGlobal` as a behavioural signal (a client that searches the server on every keystroke looks like a scraper) | The server is asked only when the user presses Tab inside the search overlay — one request per press, never on the debounce timer that drives the local index, never while typing. Hits are mirrored locally so a repeated query is answered without the server. A flood wait is shown with its length and nothing is retried. |
| The user's desktop          | A message body reaching a shell through the desktop notifier | `internal/core/notify` runs the notifier with the title and body as separate arguments (`exec`, no shell); the macOS `osascript` path builds an AppleScript string literal, so the backslash and the double quote — its only escapes — are closed before the text is put into it (`TestArgv`). The override `LAZYTG_NOTIFY_CMD` names a program, split on whitespace, never interpreted. Desktop notifications are off unless `LAZYTG_NOTIFY=desktop`. |
