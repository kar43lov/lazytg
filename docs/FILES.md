...
- 🔴 **Расширение выбирает отправитель, и оно решает, какое приложение откроется.** Прямой запуск отсечён режимом файла: скачанное лежит `0600`, без бита исполнения, поэтому `.command`, `.sh` и `.desktop` не стартуют. Не отсечены типы, которые платформа открывает и без него — `.webloc`, `.inetloc`, `.url`, `.terminal`, `.html`; они ведут себя ровно так же, как из Finder. Имя в бейдже при этом уже очищено `internal/ui/safetext`, поэтому bidi-override больше не может нарисовать `.command` как `.png`. Решение и его границы — в `docs/SECURITY.md` → threat model.
