...
## Картинка прямо в треде

`i` рисует фотографию под сообщением — в терминалах, которые умеют графику. Это дополнение к `o`, а не замена: `o` отдаёт файл системному просмотрщику (для видео это единственный честный ответ — видео не играет ни один терминал), `i` отвечает на вопрос «что там», не уводя человека из клиента.

- **Протокол — Kitty graphics** (`internal/ui/graphics`). Его понимают Ghostty, kitty и WezTerm. Не sixel: 256 цветов, без прозрачности, и кодирование, при котором фотография на проводе в разы толще своего PNG.
- **Детект по окружению, без запроса к терминалу** (`Detect`): запрос — это round-trip на старте TUI по каналу, который может быть ssh, и медленный ответ неотличим от отсутствия ответа. Цена ошибки — бейдж вместо картинки, а не сломанный экран.
- 🔴 **Внутри tmux/screen детект всегда возвращает «не умею».** Мультиплексор подменяет `TERM` и по умолчанию графику не пропускает; сообщить наружную способность терминала значило бы напечатать человеку base64 в панель. У кого passthrough настроен — `LAZYTG_IMAGE_PROTOCOL=kitty`, обратный ход `=none`.
- **Только фото.** Первый кадр видео требует декодера, которого в клиенте нет, а стикер — обычно WebP или анимация. Неправильная картинка хуже отсутствующей.
- **Ничего не рисуется само.** Тред, который тянет каждую фотографию при прокрутке, — это и медленно, и рисунок запросов, какого не даёт ни один обычный клиент; аккаунт и без того под наблюдением как неофициальный.
- **Размер — ширина треда и не больше половины его высоты.** Картинка во всю панель выталкивает разговор за экран, и человек смотрит на фото, не понимая, к чему оно.
- **Масштабирование своё, box-фильтром** (`scale`): зависимость ради косметики не берём, а усреднение исходных пикселей — ровно то, что нужно на уменьшении. Отправляется PNG, а не исходный JPEG: `f=100` — единственный сжатый формат, который принимают все реализации.
- 🔴 **Картинка живёт вне текстовой сетки, и это диктует всю остальную механику.** Терминал ставит её туда, где написан escape, и перерисовка строки пробелами её не убирает. Поэтому: у каждой картинки есть id (из id сообщения), escape начинается с удаления своих же placements — иначе прокрутка размножает фото по панели; закрытие одной картинки шлёт `DeleteImage` через `tea.Raw`, смена чата — `DeleteAll`; а `q=2` глушит подтверждения, которые иначе прилетают обратно в stdin.
