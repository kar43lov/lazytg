...
- [ ] Сообщения, сохранённые **до** обновления, остаются `[📎 …]` — это ожидаемо, зеркало не переписывается
- [ ] Два разных видео, оба присланных как `video.mp4`, скачиваются в **разные** файлы: `video.mp4` и `video (2).mp4`
