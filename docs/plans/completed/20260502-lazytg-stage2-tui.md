...
- [x] Создать `internal/app/wire.go` с `type App struct{ Client *tg.Client; Repo storage.Repo; Bus *events.Bus; Log *slog.Logger; History *sync.HistoryService; Live *sync.LiveService; Send *sync.SendService; Reconnect *sync.ReconnectManager; Updates *tg.UpdatesDispatcher; Polling *tg.PollingFallback; Keymap keymap.Keymap }`. **Implementation note:** Build разбит на две фазы — `Build(ctx, cfg)` собирает всё, что не требует активной MTProto-сессии (logger, paths, repo, bus, peers, state-repo, live, degradation, keymap), а `AttachClient(client)` поверх этого добавляет gotd-aware сервисы (History, Backfill, Sender, Updates, Reconnect). Это нужно потому что gotd `Client.Run` владеет жизненным циклом сессии — wire.Build не может его внутри держать без подкручивания owner-семантики. `RunBackground(ctx) <-chan struct{}` стартует drain-горутины LiveService + DegradationDetector. Adapters: `outgoingStoreAdapter` (sqlite.OutgoingMessage ↔ coresync.OutgoingRecord), `peerLookupAdapter` (sqlite.ErrPeerNotFound → coresync.ErrPeerUnknown), `peerResolverAdapter` (тонкий wrapper для tg.PeerResolver), `senderAdapter` (signature pass-through), `reconnectAdapter` (Connect/Ping no-op stub: реальный reconnect-loop живёт в cmd/tui.go которая владеет Client.Run)
- [x] Перенести wiring из `cmd/lazytg/cmd/runtime.go` в `internal/app/wire.go`. В `runtime.go` оставить только тонкий вызов `app.Build(...)` и `app.Run(...)`. **Note:** runtime.go в Stage 1 не содержал wiring services (только prompter/passphrase/newClient helpers); поэтому "перенос" свёлся к тому, что новые services (Live/Degradation/Sender/History) теперь живут в internal/app/wire.go, а runtime.go остаётся ответственным только за CLI-специфичные helpers (auth flow, dev/tty prompts)
- [x] Создать `cmd/lazytg/cmd/tui.go` с `tuiCmd = &cobra.Command{Use: ""... default команда без подкоманды}`. Run: создаёт `app.App` через `app.Build`, конструирует `internal/ui/app.New(deps)`, запускает `tea.NewProgram(uiApp).Run()`. Подключает `app.Bus.Subscribe()` к `program.Send()` через горутину (event-bus → tea.Msg). **Note:** bubbletea v2 использует declarative view fields для AltScreen/MouseMode (через tea.View struct в App.View()), а не WithAltScreen()/WithMouseCellMotion() program options. forwardBusEvents — отдельная функция в tui.go которая subscribe'ится к bus и вызывает program.Send для каждого event'а; sub-context отменяется в defer чтобы горутина чисто завершалась
- [x] Обновить `cmd/lazytg/cmd/root.go`: если запуск без подкоманды (по `len(os.Args) == 1` или через `cobra.Command.Run` на root) → выполнить `tuiCmd.RunE`. Иначе делегировать как обычно. **Note:** реализовано через `root.RunE = runTUI` — cobra сам выбирает между RunE root'а и subcommand RunE на основании argv; `lazytg` без подкоманды → root.RunE (TUI), `lazytg login` → login subcommand. Дополнительно: `internal/ui/app/update.go` расширен broadcast-логикой (broadcastBusEvent для events.Connection/Storage/Dialog/Message/Outgoing → routing в нужный pane; broadcastToPanes для non-key tea.Msg чтобы пане-internal load-результаты доходили до panes regardless of focus); + handleChatSelected (ChatSelectedMsg → thread.OpenChat + input.SetChatMsg + status.ChatTitle) + handleReplyRequest (RequestReplyMsg → SetReplyMsg на последнее сообщение треда)
- [x] Создать `test/e2e/tui_smoke_test.go` через `teatest.NewTestModel`: моки `Repo`, `SendService`, `HistoryService`. Сценарий:
  ...
  - **Note:** `charmbracelet/x/exp/teatest` импортирует bubbletea v1 и несовместим с charm.land/bubbletea/v2 — поэтому e2e реализован напрямую через `model.Update`/`View` calls + custom `runCmd` helper для драйвинга tea.Cmd-цепочек. Это покрывает тот же контракт высокого уровня (program loop = тонкая обёртка вокруг Update/View). Добавлены два теста: TestTUISmoke (полный flow chats load → select → focus input → send) и TestTUIBusBroadcast (events.* через broadcast в правильные pane'ы, фильтрация по chatID). isTickMsg фильтрует blink/spinner cmds (textarea cursor-blink был infinite loop без этого). fakeChatsRepo/fakeThreadRepo/fakeSender — in-memory тест-фейки
- [x] Создать `test/perf/live_latency_test.go` с **бенчмарком SLA**: запустить полный pipeline (mock gotd → updates dispatcher → bus → live service → repo → bus → UI listener), 1000 итераций, замерить latency `MessageReceived` event → final repo write. Перцентиль p95 через сортировку. **CI gate: fail если p95 >500ms**. **Result:** TestLiveUpdateLatencySLA — 1000 events, drip-feed по 32, drain wait до 10s, p95 budget 500ms. На M4: LastIngestLatency = 0ms (rounding to ms; per-event takes microseconds). BenchmarkLiveUpdateLatency = 228 ns/op. Гейт зелёный
- [x] Создать `test/perf/goroutine_leak_test.go`: запустить полный `app.App.Run` на 30 секунд с mock backends, остановить через context cancel, проверить через `goleak.VerifyNone(t)` что нет утечек горутин. **Note:** интервал укорочён до 100ms (на full run всё равно один deferred первый probe degradation detector'а — больше времени не даст полноценной нагрузки, а CI runner будет дольше); добавлен explicit `runtime.Close()` ДО `goleak.VerifyNone(...)` (defer ordering: Cleanup callbacks бегут после defer'ов, поэтому close в Cleanup промахнулся бы и database/sql.connectionOpener был бы flagged как leak); 50ms settle-window после Close для exit'а connectionOpener select-loop'а
- [x] Создать ручной smoke-чеклист в `docs/MANUAL_SMOKE.md` для Stage 2: (1) lazytg запускается без --debug → отображает чаты; (2) выбор чата → история загружается; (3) ввод "hello" + Enter → сообщение появляется в треде и приходит на телефон; (4) Ctrl+E открывает $EDITOR=vi; (5) `?` показывает help; (6) `--polling` flag запускает fallback polling. **Note:** чек-лист расширен до 16 пунктов покрывающих также: too-small terminal fallback, navigation/focus, pagination, multi-line newline (Alt+Enter), reply (Ctrl+R), $EDITOR с разными бинарями (vi/nano/cat), help overlay (?), live updates latency, polling fallback, reconnect cycle, read-only degradation (chmod 0444), graceful quit
- [x] **Verification:** `go build ./...`, `go test -race ./...`, `golangci-lint run`. Coverage gates: `go test -coverprofile=core.out ./internal/core/... && go tool cover -func=core.out | tail -1` — total ≥70%. То же для UI: `./internal/ui/... ≥50%`. Бенчмарк p95: `go test -bench=BenchmarkLiveUpdateLatency -benchtime=3s ./internal/core/sync/` — p95 <500ms (вывод проверяется глазами или парсингом). Move plan to `docs/plans/completed/20260502-lazytg-stage2-tui.md` после успешного прохода. **Result:** build clean; race-tests все зелёные (cmd, core/{config,domain,events,obs,sync}, storage/sqlite, tg, ui/{app,input,keymap,overlay,panes/chats,panes/thread,statusbar}, test/e2e, test/perf); golangci-lint 0 issues; coverage core 79.7% (≥70%), ui 80.6% (≥50%); BenchmarkLiveUpdateLatency 228 ns/op на M4. Move plan в completed выполнено отдельным шагом
