...
4. **Telegram API credentials в repo secrets — намеренно НЕ задаём.** Решение принято 18.08.2026: релизы выходят без вшитых кредов, пользователь подставляет свои.
   ...
   Механизм вшивания оставлен на месте (`.goreleaser.yaml` подставляет `LAZYTG_RELEASE_API_ID` / `LAZYTG_RELEASE_API_HASH` через `-ldflags`, `release.yml` их пробрасывает) — он просто не активирован, потому что secrets пусты. Это осознанный выбор, а не забытый шаг, и переигрывать его надо с открытыми глазами:

   - Ключ в публичном бинарнике = опубликованный ключ. `strings lazytg | grep -E '^[0-9a-f]{32}$'` печатает `api_hash`, не зная, что искать (проверено).
   - Telegram блокирует опубликованный `api_id` навсегда (`API_ID_PUBLISHED_FLOOD`), и логин отваливается **у всех пользователей релиза разом**, включая мейнтейнера. Починка = новое приложение = новый номер («each number can only have one api_id connected to it») + новый релиз.
   - Официальная инструкция Telegram: «obtain your own API id before you publish your app».

   **Следствие для тегов:** гейт `Guard Telegram API credentials` в `release.yml` приведён в соответствие с этим решением 18.08.2026. Пустая пара — штатное состояние на любом теге: job печатает `::notice` и продолжает. Падение осталось ровно на одном случае — задан **ровно один** секрет из двух: тогда в бинарнике половина вшитого слоя, и `ResolveCredentials` отвечает пользователю `build-time credentials are malformed (report this)` — тревожная ошибка про дефект, который он не может починить. До правки job валил stable-тег при пустой паре, то есть первый же `v0.1.0` был бы красным.

   🔴 **Личные креды мейнтейнера в secrets класть нельзя** — по той же причине, по которой их нет в коде. Если однажды понадобится релиз с вшитой парой, регистрировать отдельное приложение на отдельный номер, чтобы блокировка не задела личный ключ.

   Гейт против попадания ключа в git остаётся в силе: `scripts/secret-scan.sh` в lefthook `pre-commit` и job `secret-scan` в CI валят любую 32-hex строку. Что гейт **не** ловит (перечислено в шапке скрипта): намеренно разбитый или закодированный ключ, hex длиннее 32 символов, uppercase-hex и уже существующую историю. Сделать `secret-scan` **required status check** в branch protection: без этого PR с утечкой можно смержить, пока job ещё идёт.
